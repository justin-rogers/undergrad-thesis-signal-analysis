...
class WaveData:
    ...
    def __init__(self, name='', signif=0.05):
        if len(name)==0: #pick a random example if given no filename
            n=randint(1,len(anames))
            name=anames[n]
        self.name=name
        self.sig=signif #significance parameter for p diagram
        #import the data
        ...
        self.dgm, self.midi=persist0(self.fft, significance_threshhold=self.sig, high_pass=True)
        self.fft_mean=self.fft[107:].mean()
        freq=(np.argmax(self.get_fft()[107:])+107)/4
    ...
    def graph_td(self, save=False, title=None): #graphs time domain
        ...
        plt.tight_layout()
        if save:
            plt.savefig(str(time())+'_td.png')
        plt.show()
        return
        
    def graph_fft(self, save=False, title=None, a=1, b=None):
        data=self.get_fft()
        freqbins=np.fft.rfftfreq(2*len(data)-1, 1/self.sr)
        if title==None:
            title=self.name

        
        plt.figure(figsize=(12,12))
        plt.plot(freqbins[a:b], data[a:b], linewidth=1.0)
        plt.xlabel('Hz')
        plt.ylabel('Amplitude')
...
class LeanWave(WaveData):
    ...
    def __init__(self, name='', signif=.05):
        WaveData.__init__(self,name,signif)
