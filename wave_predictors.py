...
from functional_bricabrac import best_regular_guess, pshc_predict
...
try:
    if len(full_dataset)!=4060: #import data but ignore the files that shouldnt be in the dataset
...
class BasePredictor:
    ...
    def predict_all(self, wavedataset=full_dataset, output_bins=False,
        ...
        print('Accuracy for {}: {:.4%}'.format(self.predname, accuracy))
...
class ZeroX(BasePredictor):
    ...
    falling zero crossings. Ineffective but computationally cheap.
    """
    def __init__(self):
        self.predname='ZeroX'
    def predict(self, wave, output='midi'):        
...
class AutoCorr1(BasePredictor):
    ...
    def predict(self, wave, output='midi', wl=16000, wu=32000):
        A_data=wave.get_td()
...
class PersistRegular(BasePredictor):
    """
    Takes the persistence diagram and a significance threshhold.
    ...
    """
    #87.52% for sig=.05, similar for other sigs
    def __init__(self, signif=.05, l=0):
        self.predname='PersistFFT2'
        self.sig=signif
        self.l=l
    def predict(self, wave, output='midi'):
        #Finds fundamental by choosing the best candidate component from several.
        dgm, midi=wave.get_new_dgm_and_midi(self.sig)
        fund_guess=best_regular_guess(dgm, midi, self.l)
        if output=='midi':
            return fund_guess
        if output=='hz':
            return midi_to_hz(fund_guess)

class SHC(BasePredictor): #spectral harmonics correlation
    """
...
class PSHC(BasePredictor): #persistent spectral harmonics correlation
    def __init__(self, signif=.05, nbghd='thin'):
        self.nbghd=nbghd
        self.predname='PSHC'
        self.sig=signif
    def predict(self, wave, output='midi'):
        dgm,M=wave.get_new_dgm_and_midi(self.sig)
        try:
            guess=pshc_predict(dgm,M,output=output, nbghd=self.nbghd, avg=int(round((wave.fft_mean))))
        except ValueError:
            print(wave.name)
        return guess    
...
if __name__=="__main__":
    ...
    classifiers=[NaiveFFT(), ZeroX(), AutoCorr1(), PersistFFT1(.15),
                 PersistFFT2(.15), PSHC(.15)]
    for pred in classifiers:
        pred.predict_all(save=True)
    """
    """ #better tuned
    classifiers=[NaiveFFT(), ZeroX(), AutoCorr1(), PersistFFT1(.30),
                 PersistFFT2(.02), PSHC(.06)]
    for pred in classifiers:
        pred.predict_all(save=True)
    """
    """
    accs=[[], [], []]
    for param in np.linspace(.01,.40,40):
        print(param)
        p=PersistFFT1(signif=param)
        accs[0].append(p.predict_all(graph_error=False))
        q=PersistFFT2(signif=param)
        accs[1].append(q.predict_all(graph_error=False))
        r=PSHC(signif=param)
        accs[2].append(r.predict_all(graph_error=False))
    print(accs)
    """
    """
    start=time()
